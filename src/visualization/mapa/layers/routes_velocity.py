...
class RoutesVelocity(Routes):
    ...
    def paintRoute(id_avion):
        ...
        for tipo_ruta in rutas:
            for r in range(0, len(RoutesVelocity.ruta_aviones[id_avion]["rutas"][tipo_ruta]), 2):
                folium.PolyLine(
                    RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ruta_principal"][
                        RoutesVelocity.ruta_aviones[id_avion]["rutas"][tipo_ruta][r] : RoutesVelocity.ruta_aviones[
                            id_avion
                        ]["rutas"][tipo_ruta][r + 1]
                        + 1
                    ],
    ...
    def sameRoute(id_avion, timestamp):
        ...
        last_timestamp = datetime.strptime(RoutesVelocity.ruta_aviones[id_avion]['last_timestamp'], Routes.formato_fechas)

        diferencia_tiempo = new_timestamp - last_timestamp

        if ((diferencia_tiempo.total_seconds() >= 1800) and 
            RoutesVelocity.ruta_aviones[id_avion]['onGround'] and 
            RoutesVelocity.ruta_aviones[id_avion]['been_on_air']):
            return False
        return True   
    
    @staticmethod
    def addLocation(id_avion, latitud, longitud, **kwargs):
        timestamp, velocidad, onGround = kwargs.get('timestamp'), kwargs.get('velocidad', 0), kwargs.get('onGround')
        
        if (id_avion not in RoutesVelocity.ruta_aviones) or (not RoutesVelocity.sameRoute(id_avion, timestamp)):
        ...
        if RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ultima_velocidad"] is None:
            RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ultima_velocidad"] = nuevoTipoVelocidad
            RoutesVelocity.ruta_aviones[id_avion]["rutas"][nuevoTipoVelocidad].append(
                len(RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ruta_principal"]) - 1
            )
            RoutesVelocity.ruta_aviones[id_avion]["rutas"][nuevoTipoVelocidad].append(
    ...
    def updateTramosVelocidad(id_avion, nuevoTipoVelocidad):
        ...
        if nuevoTipoVelocidad == RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ultima_velocidad"]:
            RoutesVelocity.ruta_aviones[id_avion]["rutas"][nuevoTipoVelocidad][-1] = len(
                RoutesVelocity.ruta_aviones[id_avion]["rutas"]["ruta_principal"]
            )
        else:
            RoutesVelocity.ruta_aviones[id_avion]["rutas"][nuevoTipoVelocidad].append(
