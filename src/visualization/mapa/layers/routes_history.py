...
class RoutesHistory(Routes):
    capa_rutas = folium.FeatureGroup(name="Rutas Historial")
    rutas_avion = dict()
    ...
    def addLocation(id_avion, latitud, longitud, **kwargs):
        ...
        if id_avion not in RoutesHistory.rutas_avion:
            ...
            RoutesHistory.rutas_avion[id_avion].insert(0, {'ruta': [],
                 'last_timestamp': None,
                 'onGround': False,
                 'been_on_air': False
                 })
    ...
    def sameRoute(id_avion, timestamp):
        ...
        last_timestamp = datetime.strptime(RoutesHistory.rutas_avion[id_avion][0]['last_timestamp'], Routes.formato_fechas)

        diferencia_tiempo = new_timestamp - last_timestamp

        if ((diferencia_tiempo.total_seconds() >= 1800) and 
            RoutesHistory.rutas_avion[id_avion][0]['onGround'] and 
            RoutesHistory.rutas_avion[id_avion][0]['been_on_air']):
            return False
        return True   
